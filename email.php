...
echo "[?] Pilih Menu :  \n";
echo "1. Mengecek email yang sudah di buat. \n";
echo "2. Membuat email random baru. \n";
$pilihan = trim(fgets(STDIN));

if ( $pilihan == "1"){
  echo "[+] MASUKAN NAMA YANG INGIN DI CEK EMAIL .. (example : khairulimam1000)\n";
  $nama = trim(fgets(STDIN));
  echo "[*]MENCOBA MENGECEK EMAIL .. \n";
  sleep(1);
  checkmail($nama);
} elseif ( $pilihan == "2"){
  getemail();
...
function checkmail($nama){
  $email = "@boximail.com";
  $ch = curl_init();
  
  curl_setopt($ch, CURLOPT_URL, "https://getnada.com/api/v1/inboxes/".$nama."".$email."");
  ...
  $result = curl_exec($ch);
  $asu = json_decode($result,true);
  $uid = ($asu['msgs']['0']['uid']);
  
  
  $ch = curl_init();
  
  curl_setopt($ch, CURLOPT_URL, "https://getnada.com/api/v1/messages/html/".$uid."");
  curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
  curl_setopt($ch, CURLOPT_CUSTOMREQUEST, 'GET');
  ...
  //$headers[] = 'Accept-Encoding: gzip, deflate';
  $headers[] = 'Connection: close';
  $headers[] = 'Referer: https://getnada.com/message/KCjrLOXtRJHEBPaA4A4BvsAJ0WHHE3';
  $headers[] = 'Cookie: _ga=GA1.2.320054942.1660964884; _gid=GA1.2.2116287118.1661422886; _gat=1';
